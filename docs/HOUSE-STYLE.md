...
- **Card sets MUST align perfectly.** Identical heights, identical divider and button positions, identical internal rhythm across every card in a group — at every breakpoint. Misaligned cards are a P0 defect; verify by eye (and by measuring) before shipping.
  - **The specific trap (owner-caught, so treat it as a hard rule): every card in a cluster TOP-ALIGNS its content so all headings share one top baseline.** Do NOT vertically-centre cards in a cluster — `justify-center` (or `place-content-center`/`items-center`) on equal-height cards (`auto-rows-fr`/`items-stretch`/`h-full`) floats the content of longer-bodied cards to a different height, so its heading drifts out of line with its neighbours. Use `justify-start` (no `justify-center`) and let the extra height fall to the card's bottom. One cluster = one alignment rule for every card in it; never mix a centred card in among top-aligned siblings. Centring is only safe when every card's content is guaranteed the same height (e.g. a single uniform line), and even then top-align is safer.
