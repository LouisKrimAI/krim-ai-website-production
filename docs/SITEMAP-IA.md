...
## Sitemap
```
/                     Homepage — the argument; Krim in 30 seconds
...
/trust                Sovereignty & deployment — on-prem hero, posture, frameworks (honest)
/services             Engagement — deep-dive → proof of value → go-live
/company              Who Krim is — the thesis, the name, the markets
/blog                 Writing — Substack + Medium aggregated + the newsletter
/contact              Book a demo
404
```
...
## Homepage — sections
1. **Hero** — orb breathes → fades to backdrop → headline + subhead. The 30-second promise.
2. **The ceiling** — why AI stalled in regulated lending: you can't deploy what you can't prove, or risk what leaves your walls.
...
## Page section skeletons (titles only)

**/platform** — Hero (one operating system, named for what each part does) · The architecture (interactive, one line per element — the brain, the vocabulary, the co-workers, the two interfaces) · How it composes (Kriya→Karta→Kendra; surfaced through Kula & Kira) · The brain (Krim-Nyāya validates · Krim-Learn learns) · Into each part (cards → sub-pages) · CTA.

**/platform/kendra** — Hero (the brain — it validates AND it learns) · The loop (Perceive→Reason→Plan→Validate→Act) · *Power 1 — Judgment:* **Krim-Nyāya**, the gate (33 validators, three families, the honest amber path) · *Power 2 — Intelligence:* **Krim-Learn**, the real mind (how every recorded action compounds into the world model) · The supporting modules (Core, Fabric, Govern, Ledger, Sense — one line each) · The ledger (one record, three uses — evidence, meter, training signal; the single KWU mention) · CTA.
...
**/architecture** — Hero (no rip, no replace) · The runtime, technically · The substrate (memory tiers, durable orchestration, domain model) · Integration fabric (40+ connectors, event + batch) · One architecture, three geographies · FAQ · CTA.

**/research** — Hero (research at Krim) · Formalising judgment (Navya-Nyāya → 33 validators, told properly) · The lending world model (the research direction, honestly framed) · Validation science (pre-execution as a discipline) · [CONFIRM] real publications/positions, else keep as a positioning statement that can grow · CTA. *Distinct from /epistemic-ai (the buyer-facing category) and /company (the org).*

**/trust** — Hero (sovereign by construction) · Deployment (on-prem hero / hybrid / managed) · Posture (encryption, RBAC, sealed trail, customer keys) · Frameworks (honest, held vs in progress) · Encoded jurisdictions (US/UK/India) · The audit experience (ledger replay) · CTA.
...
**/blog** — Hero (the writing) · Aggregated feed (Substack + Medium, server-side, newest-first, links out; graceful empty state until feeds provided) · Newsletter sign-up (separate from the demo form).
