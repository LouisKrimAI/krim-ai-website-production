...
> **REPOSITION (2026-06, banking-led).** The lead is now **banking & financial services, lending front-and-center** — see `docs/POSITIONING.md`, which is the strategic spine this brief reconciles to. "Regulated operations" is kept as the **secondary descriptor / horizon**, never the headline. Where this section still says "regulated operations" as the lead, read "banking & financial services (lending first)"; the architecture and three-powers argument are unchanged.

## The story — three powers, one flywheel
**Krim is the operating system for banking that acts only after it has proved it should.** Lending is the flagship and the proving ground; the wider category of regulated operations is the horizon. KrimOS is the core operating system for the work — and its whole argument is three powers, locked together, not a feature list:
...
**One OS, banking-led.** KrimOS is the core — the same brain (Kendra), vocabulary (Kriya), co-workers (Karta) and interfaces (Kula, Kira) everywhere. Only the rules and use cases change per industry (carried in Krim-Fabric). **Top-level language is now "banking & financial services," with lending front-and-center** — the homepage lives under the banking umbrella and its centre of gravity is lending. The four domain pages remain: **Lending** (flagship — deeply documented, home of the World Lending Model), then the clearly-secondary **Government**, **Large Enterprise**, **MSME** (medium-sized enterprises, any industry — here the regulation-grade rigor is a *confidence* play, not a compliance one; and the bridge to the $5.7T access story). The non-banking domains are framed as *the same OS pointed at a different ruleset* — capability fit, not claimed deployments. **Invent nothing.** *(Reverses the prior rule "always regulated operations, never lending"; see POSITIONING §7.)*
