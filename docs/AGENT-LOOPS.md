...
Misaligned card sets — **specifically (owner-caught 2026-08-20): vertically-centred cards in a cluster. `justify-center` on equal-height cards (`auto-rows-fr`/`h-full`) floats longer-bodied cards' content to a different height, so headings drift out of line. TOP-ALIGN every card in a cluster (`justify-start`); one cluster, one rule; never mix a centred card among top-aligned siblings.** · "regulated operations" as a lead · dark/busy image under centred text · bland equal-weight walls of text · dense spec-sheet enumeration ("X does A, Y does B…") · number-led headings ("Eight modules…") · naming drift (Writing vs Insights, Kira vs Krimkar & Kira; pair order must be Kupa & Kula, Krimkar & Kira everywhere) · "Krim AI" · forgetting highlights · building with the dev server running · **AI-prose tells: em-dash overuse (the same "— x —" construction in every card blurb) and the "X, not Y" antithesis cliché ("software co-workers, not chatbots").** · **try-hard copy: cryptic fill-in-the-blank nav labels (the When/What/Who/Where word game), contorted headlines that strain grammar for cadence ("sharper for having run", "rules already inside them"), meta-label headlines that describe the section instead of saying something ("Why it holds together"), and strained metaphors ("a drawer of point tools").** · **under-weighted standalone lines (shipped 2026-07-22 AND again 2026-07-27, owner-caught TWICE): a centred pull-quote / closing chord / section footnote rendered at body-ish size in dim ink (`text-[1.15rem] italic text-ink-2`, or a trailing `text-body text-ink-3` pointer) floating over imagery or the ring. A standalone line that closes a section is a display moment — `text-display-2`+, `text-ink`, a canvas-halo text-shadow (or glass backing) over any busy backdrop. A small dim trailing "pointer" paragraph is usually just link-litter — DELETE it, don't shrink it.** · **feature-copy with no outcome (shipped 2026-07-27, owner-caught): a section that describes WHAT a feature is, often twice ("a twin tuned to your role… every team reads through its own twin"), and never says the IMPACT. Every feature section must name the outcome the buyer feels — the day not lost, the argument ended, the cost that stops growing — not just the mechanism. Lead the heading with the outcome ("End the argument over whose numbers are right"), support with the feature. Repetition = you have one idea dressed as two; cut to one.**

> **Two MANDATORY pre-commit self-QA greps (these defects have shipped repeatedly — automate the catch, don't trust the eye):** (1) `grep -rn "text-ink-3\|italic\|text-\[1\.[0-3]" <changed files>` → every hit that is a STANDALONE line (not an inline caption/eyebrow) must be justified or deleted/promoted; (2) for every feature section touched, ask out loud "what's the outcome sentence?" — if there isn't one in the copy, the section is not done. Run BOTH before reporting any copy/section change complete.

> **Copy must read like a person wrote it, not a model (HOUSE-STYLE §10/§11/§13).** The tone is fine; this is about the obvious tells (em-dash overuse, "X, not Y") AND about try-hard copy that sounds dumb. After drafting ANY copy, run the **language-and-sense check (HOUSE-STYLE §13)**: read it aloud, deletion test, do the first three words name what the thing IS, filing-cabinet test, is it true + concrete, boardroom test. A nav-card descriptor is a definition, not a riddle; a headline must read naturally on first pass. The `content-critic` enforces §13 and flags these — fix by saying the true thing plainly, not by adding a different flourish.
