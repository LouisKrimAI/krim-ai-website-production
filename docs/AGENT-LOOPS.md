...
> **Copy must read like a person wrote it, not a model (HOUSE-STYLE §10/§11).** The tone is fine; this is only about the obvious AI tells, mainly em-dash overuse (the same "— x —" move in every card) and the "X, not Y" cliché. Em-dashes in moderation are fine; the tell is frequency. The `content-critic` flags these; fix by varying rhythm and cutting, not by mechanically stripping punctuation or overcorrecting into tics.
