...
/**
 * Kendra — the runtime's diagrams. Two earned signature moments are animated
 * (HOUSE-STYLE §6 caps the page at ≤2); the rest are quiet static diagrams,
 * rendered in their resting/final state for everyone. Animated pieces are all
 * whileInView / once, GPU-only (opacity · transform · colour) and
 * reduced-motion-safe: useReducedMotion settles each to its end state instantly.
 *
 *   OrbBrainCard    — the hero's verbatim card; the two power-names, resting. (static)
 *   LoopFlow        — the five-step path with the Validate gate lit. (static)
 *   ValidationGate  — three families check an action; pass glows mint, one
 *                     amber peels off to an exception lane. (signature)
 *   WorldModelField — a connected point field + a rising curve, at rest. (static)
...
// =============================================================================
// OrbBrainCard — hero verbatim card; the two power-names rest in their colours.
// Quiet static (no scroll reveal) — the motion budget is spent on the two
// signature devices below.
// =============================================================================

export function OrbBrainCard() {
  return (
    <div className="glass max-w-[440px] p-7 md:p-8">
      ...
      <div className="mt-6 flex items-center gap-3 font-mono text-eyebrow uppercase">
        <span style={{ color: CYAN }}>Krim-Nyāya</span>
        <span className="text-ink-3" aria-hidden>
          ·
        </span>
        <span style={{ color: MINT }}>Krim-Learn</span>
      </div>
    </div>
  )
}
...
export function LoopFlow() {
  // Quiet static diagram — the resting state of the loop. Labels stay legible
  // on narrow screens via a horizontal-scroll wrap with a min width.
    ...
    <div className="glass overflow-hidden p-6 md:p-8">
      <div className="-mx-2 overflow-x-auto px-2">
        <svg
          ...
          role="img"
          aria-label="The action loop: Perceive, Reason, Plan, Validate, Act — the Validate gate is the step the others serve."
        >
          {/* the rail */}
          ...
          {LOOP_STEPS.map((label, i) => {
            ...
            return (
              <g key={label}>
                <circle cx={x} cy="70" r={isGate ? 5 : 3.5} fill={settled} />
                <text
                  x={x}
                  y="132"
                  textAnchor="middle"
  ...
  return (
    <div className="glass overflow-hidden p-6 md:p-8">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-5 font-mono text-eyebrow uppercase">
      ...
      <p className="mt-5 font-sans text-caption leading-relaxed text-ink-2">
...
export function WorldModelField() {
  // Quiet static diagram — the model in its enriched resting state (points
  // connected, curve drawn). No accrual animation; motion budget is reserved
  // for the two signature devices. The 400-wide viewBox keeps labels legible
  // on narrow screens without a scroll wrap.
  ...
  return (
    <div className="glass p-6 md:p-7">
      ...
      <svg
        ...
        className="w-full"
        role="img"
        aria-label="A model of the operation enriched by actions: points connected into structure."
      >
        {/* the connections that make it a model, not noise */}
        ...
        {/* the points at their resting brightness */}
      ...
      <div className="mt-5 border-t border-soft pt-5">
        ...
        <p className="mt-4 font-sans text-caption leading-relaxed text-ink-2">
          ...
          {targets.map((t, i) => (
            <g key={t.name}>
              <motion.path
                d={streamPath(t.y)}
                fill="none"
                stroke={i === 1 ? MINT : 'var(--border-strong)'}
                strokeWidth="1.3"
        ...
        <ul className="space-y-5">
          {jobs.map((j, i) => (
            ...
            >
              ...
              <p className="mt-2 font-sans text-caption leading-relaxed text-ink-2">{j.body}</p>
            </motion.li>
          ))}
        </ul>
      </div>
