...
3. **Alignment (the 1–2px hunt).** Left edges of stacked elements align to the same axis; centred elements are truly centred; accent bars / rules align to the text they head; icon and text share a baseline; grid columns line up; no sub-pixel drift between sections that should share a container width.
   - **Card-cluster heading baseline (a classic, owner-caught miss — check EVERY grid/row of sibling cards).** In a cluster of sibling cards, every card's heading (and its top element: rail, eyebrow, icon) MUST sit on the *same top baseline*. The failure mode is `justify-center` / `place-content-center` / `items-center` on equal-height cards (`auto-rows-fr`, `items-stretch`, or `h-full`) whose bodies differ in length: the card with more text stays vertically centred, so its heading floats up or down, out of line with its neighbours. **Measure the first-text y-offset of each card in the cluster; if they differ, it is P0 (glaring) or P1.** The fix is always top-align: `justify-start` (drop `justify-center`) and let the unequal height fall to the bottom. Also flag any cluster that *mixes* the rule — one centred card among top-aligned siblings (e.g. a highlighted "+ bonus" tile). One cluster, one alignment rule, applied to every card.
