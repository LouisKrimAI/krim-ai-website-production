    ...
    <footer className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-site px-6 py-16 md:px-10 md:py-20">
        ...
        <nav className="mt-12 grid grid-cols-2 gap-x-8 gap-y-8 md:grid-cols-4" aria-label="Footer">
          {NAV_COLUMNS.map((col) => (
            <div key={col.title}>
              {/* tap floor: every link is a padded block (no invisible halos,
                  which overlap neighbours); the list trades its gap for link
                  padding so the rhythm holds while each row's hit box grows */}
              <Link
                href={col.href}
                className="inline-block py-1.5 font-mono text-[11px] uppercase tracking-[0.18em] text-ink-3 transition-colors hover:text-mint"
              >
                {col.title}
              </Link>
              <ul className="mt-1.5 flex flex-col">
                {col.rows.map((r) => (
                  <li key={r.href}>
                    <Link
                      href={r.href}
                      className="inline-block py-[7px] font-sans text-[14.5px] text-ink-2 transition-colors hover:text-ink"
                    >
                      {r.label}
                    </Link>
                  </li>
        ...
        <div className="mt-14 flex flex-wrap items-center justify-between gap-x-8 gap-y-5 pt-8">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              className="-m-1 p-3 text-ink-2 transition-colors hover:text-mint"
            >
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-1.5 font-mono text-[11px] tracking-[0.06em] text-ink-3">
          ...
          <a href="/privacy" className="inline-block py-2 transition-colors hover:text-ink-2">
            Privacy
          </a>
          <a href="/terms" className="inline-block py-2 transition-colors hover:text-ink-2">
            Terms
          </a>
        </div>
      </div>
